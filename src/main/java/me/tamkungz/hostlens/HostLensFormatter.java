...
public final class HostLensFormatter {
    ...
    public static String pretty(HostSnapshot snapshot) {
        ...
        if (snapshot.disks().isEmpty()) {
            ...
            for (DiskInfo disk : snapshot.disks()) {
                ...
                if (!isUnknown(disk.driveType())) {
                    builder.append(' ').append(disk.driveType());
                }
                if (!isUnknown(disk.mediaType())) {
                    builder.append('/').append(disk.mediaType());
                }
                if (disk.rootMount()) {
                    builder.append(" root");
                }
                ...
                builder.append("  Space      : ")
                        .append(formatBytes(disk.usedBytes())).append(" used / ")
                        .append(formatBytes(disk.totalBytes())).append(" total");
                if (disk.usedPercent() >= 0) {
                    builder.append(" (").append(formatPercent100(disk.usedPercent())).append(')');
                }
                builder.append(" / ").append(formatBytes(disk.usableBytes())).append(" usable\n");

                if (!isUnknown(disk.device())) {
                    builder.append("  Device     : ").append(disk.device()).append('\n');
                }
                if (!disk.volumeName().isBlank()) {
                    builder.append("  Volume     : ").append(disk.volumeName()).append('\n');
                }
                if (disk.removable() || disk.network() || disk.virtual() || disk.ssd()) {
                    builder.append("  Flags      : removable=").append(disk.removable())
                            .append(", network=").append(disk.network())
                            .append(", virtual=").append(disk.virtual())
                            .append(", ssd=").append(disk.ssd())
                            .append('\n');
                }
            }
        }
    ...
    private static String formatPercent100(double value) {
        if (value < 0 || Double.isNaN(value) || Double.isInfinite(value)) {
            return "unknown";
        }
